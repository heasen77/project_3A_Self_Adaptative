...
public class Simulation_Opti_Test {
	...
	public Simulation_Opti_Test(int h, int unit, int[] initTL1, boolean[] initTL2, int[] initCar){
	...
	public void Simulation(int xMin, int xStop, int vSL) {
		...
		IntVar aMinimize = model.intVar("aMininimize", -100, max(abs(aMin), aMax));
		//model.min(aMinimize, aVar).post(); //aMinimize < aVar

		IntVar aObjective = model.intVar("aObjective", -100, max(abs(aMin), aMax) + aMax - aMin);
		aMinimize.add(aDelta).eq(aObjective).post();
		//aMininize + aDelta = aObj
		
		
		ParetoOptimizer po = new ParetoOptimizer(Model.MAXIMIZE, new IntVar[]{vObjective});
